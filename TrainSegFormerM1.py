#!/usr/bin/env python3
"""
SegFormer M1 - Water Bottle Segmentation Training
Dataset: All-1.json (polygon annotations, Water_Bottle class)
Mirrors the DETR M1 (fine_tune.py) approach using SegFormer.
Segmentation masks -> bounding boxes at inference time.
"""
...
# Label mapping: 0 = background, 1 = bottle
...
class BottleSegDataset(Dataset):
    """Water bottle segmentation dataset from COCO polygon annotations."""
    ...
    def __init__(self, images_dir, annotations_file, processor,
        ...
        # Map image_id -> info (only files that exist on disk)
        self.img_id_to_info = {}
        ...
        # Keep only Water_Bottle category (id=1 in All-1.json)
        bottle_cat_ids = {c["id"] for c in coco["categories"] if "bottle" in c["name"].lower()}

        # Group annotations by image, segmentation only
        self.img_to_anns = {}
    ...
    def _polygon_to_mask(self, polygons, img_size):
        """COCO polygon list -> binary numpy mask (H x W)."""
    ...
    def __getitem__(self, idx):
        ...
        # Build semantic mask: 0=background, 1=bottle
        mask = np.zeros((orig_h, orig_w), dtype=np.uint8)
        for ann in anns:
            try:
                poly_mask = self._polygon_to_mask(ann["segmentation"], image.size)
...
def main():
    print("=" * 60)
    print("SEGFORMER M1 - WATER BOTTLE SEGMENTATION")
    print("=" * 60)
    ...
    # Build train/val split from JSON
    print("\nCreating train/val split from All-1.json...")
    with open(ANNOTATIONS_FILE) as f:
        coco = json.load(f)
    ...
    print(f"\nStarting M1 training: {EPOCHS} epochs")
